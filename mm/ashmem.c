...
static int ashmem_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	if (mutex_trylock(&ashmem_mutex) == 0) {
		if (current->mm && ashmem_mutex.owner && ashmem_mutex.owner->task
			&& (ashmem_mutex.owner->task->mm == current->mm) 
			&& !list_empty(&current->mm->mmap_sem.wait_list)) {
			/* search wait list of current->mm->mmap_sem to see
			 * if the ashmem_mutex owner is on it. */
			if(rwsem_task_in_wait_list(&current->mm->mmap_sem, ashmem_mutex.owner->task)) {
				/* this will deadlock the system so lets just return -EINTR */
				pr_info("!WARN! ashmem_mmap returned -EINTR to avoid deadlock!\n");
				return -EINTR;
			}
		}
		mutex_lock(&ashmem_mutex);
