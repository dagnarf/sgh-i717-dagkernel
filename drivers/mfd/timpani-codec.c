...
*/
#define TIMPANI_REGSET_ARRAY_SIZE 177
...
static int adie_codec_write(u8 reg, u8 mask, u8 val)
{
	...
	if (reg > TIMPANI_A_CDC_COMP_HALT) {
		pr_info("register number is out of bound for shadow"
					" registers reg = %d\n", reg);
		new_val = (val & mask) ; 
...
//remove array overrun possibility :0xEF -> TIMPANI_REGSET_ARRAY_SIZE  	for (i = 0; i < 0xEF; i++) {
				...
				u32 requested_freq)
{
	...
	for (i = 0; i < profile->setting_sz; i++) {
		if (profile->settings[i].freq_plan >= requested_freq) {
			...
			pr_err("%s : profile = 0x%x, freq_plan = %d\n", __func__, profile, profile->settings[i].freq_plan);
		}
	}
	return rc;
}
