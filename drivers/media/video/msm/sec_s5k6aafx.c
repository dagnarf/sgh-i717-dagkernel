...
static void  s5k6aafx_get_exif(void)
{
	...
	s5k6aafx_i2c_write_32bit(s5k6aafx_client->addr, 0xFCFCD000);					
	s5k6aafx_i2c_write_32bit(s5k6aafx_client->addr, 0x002C7000);
	s5k6aafx_i2c_write_32bit(s5k6aafx_client->addr, 0x002E14D0); // ExpoureTime
	s5k6aafx_i2c_read(0x0F12, &read_value1); 		

	temp = 500000/read_value1;

	s5k6aafx_ctrl->exif_exptime = temp; 
	CAM_DEBUG("read_value1 = %d, ExposureTime = %d",read_value1,s5k6aafx_ctrl->exif_exptime); 
	

	/* Get ISO */
	s5k6aafx_i2c_write_32bit(s5k6aafx_client->addr, 0xFCFCD000);	
	s5k6aafx_i2c_write_32bit(s5k6aafx_client->addr, 0x002C7000);
	s5k6aafx_i2c_write_32bit(s5k6aafx_client->addr, 0x002E14C8); // ISO
	s5k6aafx_i2c_read(0x0F12, &read_value1); 	
	...
	s5k6aafx_ctrl->exif_iso = iso_table[index];
	CAM_DEBUG("read_value1 = %d, gain = %d, ISO = %d",read_value1, gain, s5k6aafx_ctrl->exif_iso); 
}
...
static int s5k6aafx_sensor_init_probe(const struct msm_camera_sensor_info *data)
{
	...
	gpio_tlmm_config(GPIO_CFG(41, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_16MA), GPIO_CFG_ENABLE); //reset
	gpio_tlmm_config(GPIO_CFG(42, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_16MA), GPIO_CFG_ENABLE); //stanby
	...
	printk("2.set MCLK 24MHz");
	msm_camio_clk_rate_set(24000000);
	mdelay(5);
	
	printk("3. CAM_VGA_RST HIGH ");
	gpio_set_value_cansleep(41, 1);// VT RST
	udelay(50);

	printk("4. CAM_5M_RST LOW ");
	gpio_set_value_cansleep(50, 0);// 5M RST
	mdelay(100);
	
	//printk("3. CAM_5M_STBY HIGH ");
	//gpio_set_value_cansleep(49, 1);// 5M STBY
	//mdelay(100);
...
//VT RESET	
	...
	gpio_set_value_cansleep(41, 0);// VT RST
	mdelay(1);


//VT STBY	
	printk(" 2. CAM_VT_STBY OFF");
	gpio_set_value_cansleep(42, 0);// VT  RST
	mdelay(1);

//5M RST	
	printk(" 3. CAM_5M_RST OFF");// 5M RST
	gpio_set_value_cansleep(50, 0);// VT  RST
	mdelay(1);	
...
static int s5k6aafx_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (IS_ERR(s5k6aafx_dev)) {
       ...
       if (device_create_file(s5k6aafx_dev, &dev_attr_type) < 0) {
...
#ifdef FACTORY_TEST
	device_remove_file(s5k6aafx_dev, &dev_attr_type);
#endif
