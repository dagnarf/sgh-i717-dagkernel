...
static int sr200pc20m_i2c_read(unsigned short subaddr, unsigned short *data)
{
	int ret;
	unsigned char buf[2];
	struct i2c_msg msg = { sr200pc20m_client->addr, 0, 2, buf };
...
static void  sr200pc20m_get_exif(void)
{
	...
	sr200pc20m_i2c_write_16bit(sr200pc20m_client->addr, 0xFCFC);					
	sr200pc20m_i2c_write_16bit(sr200pc20m_client->addr, 0x002C);
	sr200pc20m_i2c_write_16bit(sr200pc20m_client->addr, 0x002E); // ExpoureTime
	sr200pc20m_i2c_read(0x0F, &read_value1); 

	temp = 500000/read_value1;

	sr200pc20m_ctrl->exif_exptime = temp; 
	CAM_DEBUG("read_value1 = %d, ExposureTime = %d",read_value1,sr200pc20m_ctrl->exif_exptime); 
	

	/* Get ISO */
	sr200pc20m_i2c_write_16bit(sr200pc20m_client->addr, 0xFCFC);	
	sr200pc20m_i2c_write_16bit(sr200pc20m_client->addr, 0x002C);
	sr200pc20m_i2c_write_16bit(sr200pc20m_client->addr, 0x002E); // ISO
	sr200pc20m_i2c_read(0x0F, &read_value1); 
	...
	CAM_DEBUG("read_value1 = %d, gain = %d, ISO = %d",read_value1, gain, sr200pc20m_ctrl->exif_iso); 
}
#endif
