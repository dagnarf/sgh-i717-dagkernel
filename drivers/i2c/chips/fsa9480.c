...
#define ADC_GND			0x00
#define ADC_MHL			0x01
#define ADC_DOCK_PREVIOUSSONG	0x04
#define ADC_DOCK_NEXTSONG	0x07
#define ADC_DOCK_VOL_DN		0x0a
#define ADC_DOCK_VOL_UP		0x0b
#define ADC_DOCK_PLAYPAUSE	0x0d
...
enum {
	...
	DOCK_KEY_PLAYPAUSE_PRESSED,
	DOCK_KEY_PLAYPAUSE_RELEASED,
	DOCK_KEY_PREVIOUSSONG_PRESSED,
	DOCK_KEY_PREVIOUSSONG_RELEASED,
	DOCK_KEY_NEXTSONG_PRESSED,
	DOCK_KEY_NEXTSONG_RELEASED,
};
...
void EnableFSA9480Interrupts(void)
{
	...
	i2c_smbus_read_byte_data(client, FSA9480_REG_INT1);
	i2c_smbus_read_byte_data(client, FSA9480_REG_INT2);
...
static int fsa9480_handle_dock_vol_key(struct fsa9480_usbsw *info, int adc)
{
	...
	if (adc == ADC_OPEN) {
		...
		case DOCK_KEY_VOL_DOWN_PRESSED:
			...
			break;
		case DOCK_KEY_PLAYPAUSE_PRESSED:
			code = KEY_PLAYPAUSE;
			state = 0;
			info->previous_key = DOCK_KEY_PLAYPAUSE_RELEASED;
			break;
		case DOCK_KEY_PREVIOUSSONG_PRESSED:
			code = KEY_PREVIOUSSONG;
			state = 0;
			info->previous_key = DOCK_KEY_PREVIOUSSONG_RELEASED;
			break;
		case DOCK_KEY_NEXTSONG_PRESSED:
			code = KEY_NEXTSONG;
			state = 0;
			info->previous_key = DOCK_KEY_NEXTSONG_RELEASED;
			break;
		default:
			return 0;
		}
	...
	if (pre_key == DOCK_KEY_NONE) {
		if (adc != ADC_DOCK_VOL_UP && adc != ADC_DOCK_VOL_DN
			&& adc != ADC_DOCK_PLAYPAUSE && adc != ADC_DOCK_PREVIOUSSONG
			&& adc != ADC_DOCK_NEXTSONG)
			return 0;
	}
	...
	case ADC_DOCK_VOL_DN:
		...
		break;
	case ADC_DOCK_PLAYPAUSE:
		code = KEY_PLAYPAUSE;
		state = 1;
		info->previous_key = DOCK_KEY_PLAYPAUSE_PRESSED;
		break;
	case ADC_DOCK_PREVIOUSSONG:
		code = KEY_PREVIOUSSONG;
		state = 1;
		info->previous_key = DOCK_KEY_PREVIOUSSONG_PRESSED;
		break;
	case ADC_DOCK_NEXTSONG:
		code = KEY_NEXTSONG;
		state = 1;
		info->previous_key = DOCK_KEY_NEXTSONG_PRESSED;
		break;
	case ADC_DESKDOCK:
		if (pre_key == DOCK_KEY_VOL_UP_PRESSED) {
			...
			info->previous_key = DOCK_KEY_VOL_DOWN_RELEASED;
//		} else {
		} else if (pre_key == DOCK_KEY_PLAYPAUSE_PRESSED) {
						code = KEY_PLAYPAUSE;
						state = 0;
						info->previous_key = DOCK_KEY_PLAYPAUSE_RELEASED;
		} else if (pre_key == DOCK_KEY_PREVIOUSSONG_PRESSED) {
				code = KEY_PREVIOUSSONG;
				state = 0;
				info->previous_key = DOCK_KEY_PREVIOUSSONG_RELEASED;
		} else if (pre_key == DOCK_KEY_NEXTSONG_PRESSED) {
				code = KEY_NEXTSONG;
				state = 0;
				info->previous_key = DOCK_KEY_NEXTSONG_RELEASED;
		} else {
