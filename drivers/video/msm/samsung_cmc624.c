...
int load_tuning_data(char *filename)
{
	...
	pr_debug("[CMC624:INFO] : Loading Tuning Value's Count : %d", num);
	cmc624_tune(num);
...
//		bInit = 1;
		...
		if(!bRead){
			...
			read_ldi_reg(LDI_ID_ADDR, id_buffer, 8);//joann_test
...
//extern int state_lcd_on; //mipi_s6e8ab0_wxga.c //joann_test
...
//	if(state_lcd_on)//joann_test
	...
	//		printk("cmc624 add 0x%x = 0x%x\n",cmc_read_add[i], data[i]);
			...
			else { if((compare_data[3] == data[3])&&(i == 3))
				    {	error_count = 0;
				  //  	printk("cmc esd timer - count : %d\n", error_count);
					 }
				//	printk("cmc624 add 0x%x = 0x%x\n", cmc_read_add[i], data[i]);
