...
static void update_LCD_SEQ_by_id(struct lcd_setting *destLCD)
{
	...
	if( destLCD->isSmartDimming && s6e8aa0_lcd.isSmartDimming_loaded )
	{
		...
		if( destLCD->factory_id_line == lcd_id_a2_sm2_line )
		{
		switch( destLCD->AID )
		{
			case 1:
...
void update_id( char* srcBuffer, struct lcd_setting *destLCD )
{
	...
	switch( checkValue )
	{
		...
		break;
		case 0xA2:
			destLCD->factory_id_line = lcd_id_a2_sm2_line;
			destLCD->isSmartDimming = TRUE;
			LOG_ADD( " A2SM2-Line" );
		break;
		default :
...
device_attribute *attr, char *buf)
{
	...
	case lcd_id_a2_sm2_line: 
		cntBuffer += sprintf(pBuffer +cntBuffer, " A2SM2");
	break;
